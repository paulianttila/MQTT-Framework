...
class MyApp:
    ...
    def init(self, callbacks: Callbacks) -> None:
        ...
        self.interval_trigger_counter_metric = Counter('interval_trigger_counter', '', registry=self.metrics_registry)
        self.manual_trigger_counter_metric = Counter('manual_trigger_counter', '', registry=self.metrics_registry)
    ...
    def mqtt_message_received(self, topic: str, message: str) -> None:
        ...
        if topic == 'request':
            ...
            self.publish_value_to_mqtt_topic('config_variable_response', self.config["TEST_VARIABLE"])
        elif topic == 'healthy_check_state':
            self.healthy_check_state = message.lower() in ("true")
            self.publish_value_to_mqtt_topic('healthy_check_state_response', self.healthy_check_state)
    ...
    def do_update(self, trigger_source: TriggerSource) -> None:
        ...
        if trigger_source == TriggerSource.MANUAL:
            ...
            self.publish_value_to_mqtt_topic('manual_trigger_counter', self.manual_trigger_counter)
            ...
            self.publish_value_to_mqtt_topic('interval_trigger_counter', self.interval_trigger_counter)
